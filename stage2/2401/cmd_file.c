...
int do_filesystem_dir(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	{	
		...
		while(1)
		{
			...
			strcat(strFullPath, path);
			//printf("%s", path);//打印路径	//printf("/");
			if (path[strlen(path)-1] != '/')
				strcat(strFullPath, "/");
			strcat(strFullPath, fn);

			printf("<dir>");
			printf("%s",  strFullPath);//打印文件名	  

			FIL fp;
		 	if (FR_OK != f_open(&fp, strFullPath, FA_READ))
			{
				printf("\terror");
			}
			else
			{
			printf("\tsize:%dbytes", f_size(&fp));
			}
			printf("</dir>\n");
		} 
	}	  
...
U_BOOT_CMD(
	fsappend,	CFG_MAXARGS,	1,	do_filesystem_append,
 	"fsappend  - append content to file on disk \n",
 	"[command ...]\n"
 	"    - show help information (for 'command')\n"
 	"'help' prints online help for the monitor commands.\n\n"
 	"Without arguments, it prints a short usage message for all commands.\n\n"
...
int do_filesystem_generated(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	if (argc != 2)
	{
		printf("args number error!\n");
	...
	if ( FR_OK != f_truncate(&file) )
	{
		printf("file truncate error!\n");
		return 0;
	}

	char str[] = "abcdeddfdsf jfksdjk rer34234\n";
	int count = 0;
	while (FR_OK == f_write(&file, str, sizeof(str), &count))
	{
		// 扫描串口数据，根据数据决定行为
		int bFlag = 0;
		while(tstc())
		{
			...
			case 0x04:        /* ^D - break   */
				printf("<state><running>fsgen</running></state>\n");
				break;
			};
		}
...
U_BOOT_CMD(
	fsgen,	CFG_MAXARGS,	1,	do_filesystem_generated,
 	"fsgen  - generate content to file on disk \n",
 	"[command ...]\n"
 	"    - show help information (for 'command')\n"
 	"'help' prints online help for the monitor commands.\n\n"
 	"Without arguments, it prints a short usage message for all commands.\n\n"
